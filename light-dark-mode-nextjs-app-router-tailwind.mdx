...
Applying light and [dark mode](https://tailwindcss.com/docs/dark-mode) themes with TailwindCSS is easy. However, if you want to allow users to toggle between light and dark mode themes while also identifying the system preference setting at load time and avoid a page flicker, things get a little more complicated.
...
## 2. Toggling dark mode manually in TailwindCSS
...
You must keep track of the mounted state because you cannot use the `setTheme` function from the `useTheme` hook unless you know you are on the client-side. Without this check, you will get a hydration mismatch warning when rendering on the server. This is because the server will not know the user theme preference. 

You need to check this inside of the `useEffect` hook that only runs on component mount when it has an empty dependency array. 
...
You should now have a Next.js website that not only applies the user system preference, but also remembers any user changes to the light and dark mode theme preference specifically for your site. Therefore, a user may have a system preference of light mode, but still choose a default dark mode for your website.  
