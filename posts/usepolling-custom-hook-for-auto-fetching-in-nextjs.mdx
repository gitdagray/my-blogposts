...
The `useEffect` hook has an empty dependency array `[]` so it will only run once on mount. However, the interval will continue to run at the defined interval until the component is unmounted.
...
Then, inside the body of the component, I call the `usePolling` function with a desired interval of 3000 milliseconds (3 seconds). 

The `usePolling` function will call `router.refresh()` every 3 seconds. 
...
Note: In this example, there is a 1 in 3 chance that the data will be the same as the last time it was fetched. That said, it should change often enough to see the polling in action.
