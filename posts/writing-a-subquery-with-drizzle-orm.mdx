---
title: 'Writing a SQL Subquery with Drizzle ORM'
description: 'How to write a SQL subquery with Drizzle ORM.'
date: '2024-01-14'
modified: '2024-01-14'
tags: ['sql','drizzle','orm']
cover_image: false
---

I need to write a SQL select query that joins a couple of tables. 

Fairly common stuff so far. 
...
```
I need to add in a max ship week value from a Shipping Records table. This table has a many-to-many relationship with the Plant table. 

Now, I create the ship week subquery _above the main query_: 
```ts 
import { db } from "@/index"
import { plant, genus } from "@/schema"
...
Next, I can add the subquery to the main query with a left join: 
```ts 
const plantData = await db.select({
        ID: plant.plantVarietyInfoId,
        Genus: genus.description,
